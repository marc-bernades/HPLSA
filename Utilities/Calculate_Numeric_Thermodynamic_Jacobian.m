...
for ii = 1:length(c_v)
    ...
    if ~strcmp(HP_model, 'CoolProp') && ~strcmp(HP_model, 'RefProp')
    ...
    else % CoolProp / RefProp

        delta_T        = epsilon*max(abs(T(ii)), 1);
        delta_rho      = epsilon*max(abs(rho(ii)), 1);

        P_0(ii) = py.CoolProp.CoolProp.PropsSI("P","T",T(ii),"D",rho(ii),Substance);

        %% Partial temperature derivatives
        T_prev = T(ii) - delta_T; %T(ii)*stencil_weight;
        T_next = T(ii) + delta_T; %T(ii)*stencil_weight;

        c_v_prev    = py.CoolProp.CoolProp.PropsSI("CVMASS","T",T_prev,"P",P_0(ii),Substance);  	% Specific heat at constant pressure [J/kg/K]
        c_v_next    = py.CoolProp.CoolProp.PropsSI("CVMASS","T",T_next,"P",P_0(ii),Substance);  	% Specific heat at constant pressure [J/kg/K]
        ...
        % dmu / drho at T1
        rho_prev = rho(ii) - delta_rho;
        rho_next = rho(ii) + delta_rho;
